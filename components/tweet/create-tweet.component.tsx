...
const CreatePost: React.FC = () => {
  ...
  const [tweetMsg, addTweetMsg] = useState('');
  

  async function addtweet(){
    let session:any = await getSession();
    await supabase.from('tweets').insert({text:tweetMsg,user_id:session.user.id});
    addTweetMsg('')
  }
  ...
  return (<>
    <div className="flex">
      ...
      <div className="flex-1 px-2 pt-2 mt-2">
        <textarea
          value={tweetMsg}
          onChange={(ev)=>addTweetMsg(ev.target.value)}
    ...
    <div className="flex">
      ...
      <div className="flex-1">
        <button onClick={()=>addtweet()} className="bg-blue-400 mt-5 hover:bg-blue-600 text-white font-bold py-2 px-8 rounded-full mr-8 float-right">
          Tweet
        </button>
      </div>
    </div>
