...
const TwitterPost: React.FC = async () => {

  const tweets = (await supabase.from('tweets').select('*, user:users(*)')).data;
  console.log(tweets)
    ...
    <div className="flex flex-shrink-0 p-4 pb-0">
      <a href="#" className="flex-shrink-0 group block">
        <div className="flex items-center">
          <div>
            <Image width={10} height={10} className="inline-block h-10 w-10 rounded-full" src={tweet?.user?.image || ''} alt="" />
          </div>
          <div className="ml-3">
            <p className="text-base leading-6 font-medium text-white">
              {tweet.user.name}
              <span className="text-sm leading-5 font-medium text-gray-400 group-hover:text-gray-300 transition ease-in-out duration-150">
                @{tweet.user.email} . 16 April
              </span>
            </p>
          </div>
        </div>
